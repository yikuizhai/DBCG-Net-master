...
class FeatureFusionModule(BaseModule):
    ...
    def __init__(self,
        ...
        self.conv0 = ConvModule(
            512, out_channels, 1, norm_cfg=norm_cfg, act_cfg=act_cfg)
    ...
    def forward(self, spatial_inputs, context_inputs1, context_inputs):
        spatial_inputs = F.interpolate(spatial_inputs, size=context_inputs.size()[2:], mode='bilinear',
                                       align_corners=True)
        inputs = torch.cat([spatial_inputs, context_inputs], dim=1)
        inputs = F.interpolate(inputs, size=context_inputs1.size()[2:], mode='bilinear',
                               align_corners=True)
        inputs = torch.cat([inputs, context_inputs1], dim=1)
        x = self.conv0(inputs)
...
class CascadeFeatureFusion(nn.Module):
    ...
    def __init__(self, low_channels, high_channels, out_channels, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.conv_high = nn.Sequential(
            nn.Conv2d(high_channels*4, out_channels, 1, bias=False),
            norm_layer(out_channels)
        )
...
@MODELS.register_module()
class DBCGContextPathNet(BaseModule):
    """DBCGNet with Context Path. The `outs` below is a list of three feature
    ...
    def forward(self, x):
        ...
        spatial_out, x_cls = self.spatialpath(x, outs[0])
        ...
        feat_fuse = self.ffm(spatial_out, outs[0], arms_out[1])
